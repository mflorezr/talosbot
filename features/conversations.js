...
module.exports = function (controller) {
  ...
  onboarding.say(
    "Hey! Are you ready to fill out the *Front end Training* report now?"
  );
  ...
  onboarding.addMessage(
    "Nice, this is your update for *Front end Training*. _Type `cancel` to stop or step `back` with back_",
    "yesterday_thread"
  );
  ...
  onboarding.addMessage(
    "Well done! This is all, you can continue with your work :muscle:",
    "end_thread"
  );
  ...
  onboarding.after(async (results, bot) => {
    axios
      ...
      .then((response) => {
        axios
          .get(
            `https://talosbotstrapi.herokuapp.com/slack-users/find/${results.user}`,
            {
              headers: {
          ...
          .then((user) => {
            if (results.today && results.yesterday && results.blockers) {
              axios
                .post(
                  "https://talosbotstrapi.herokuapp.com/reports",
                  {
                    Today: results.today,
                    Yesterday: results.yesterday,
                    Blockers: results.blockers,
                    slack_user: user.data.id,
                  },
                  {
                    headers: {
                      Authorization: `Bearer ${response.data.jwt}`,
